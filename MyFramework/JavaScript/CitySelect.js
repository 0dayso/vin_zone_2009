 ...
 *
 */
function csInit(viewID, targetID) {
	if(_csOptionDiv==null) {
		csCreateDiv();
		var b = document.getElementsByTagName('BODY');
		b[0].appendChild(_csOptionDiv);
	}
	try {
		var tmp = document.getElementById(viewID);
		tmp.onpropertychange = csSearchVal;
		tmp.autocomplete = "off";
		tmp.onkeyup = keyValue;
		tmp.value = _csHint;
		tmp.onblur = csOnBlur;
		tmp.onfocus = csOnFocus;
		document.getElementById(targetID);
		csSetTarget(viewID, targetID);
		_csMovekey = 0;
	}catch (e) {
		alert(e.message);
	}
