//机票订单页面专用 add by liyao 2007-11-07
...
bsGCCombox.prototype.getGCXML = function() {
	...
	xmlFile=xmlFile.replace("@0",this.userInputObj.value);
...
bsGCCombox.prototype.createGCCombox=function() {
  ...
  ComboxDiv.style.height=this.gcComboxTable.offsetHeight;
  return true;
}
bsGCCombox.prototype.showGCCombox = function() {
	...
	curEle.style.width = this.userInputObj.style.width ;//this.gcComboxTable.offsetWidth+5;
	...
	//curEle.style.width=this.gcComboxTable.offsetWidth+2;
	if (curEle.offsetHeight>300) curEle.style.height=300;		
    curEle.style.height = curEle.style.posHeight + 2 ;
...
bsGCCombox.prototype.blurInputObj=function() {
	...
	if (!isGCComboxEvent) hideGCCombox();	
}
bsGCCombox.prototype.selectGCByMouse=function() {
	...
	//如果是公司备注，则追加
	if(this.userInputObj.id == "tCustomerRemark" )
	...
	var eSrc = window.event.srcElement;
	window.event.cancelBubble=true;
	var Addresses = eSrc.innerText.split("|");
    if( Addresses.length < 3 )
    {
        bsSendContactName.value=Addresses[0];
    }
    else
    {
        bsSendAddress.value = Addresses[2] ; //送票地址
        if( bsSendMobile.value == "" )
                bsSendMobile.value =Addresses[1] ; //联系电话
        if( bsSendContactName.value == "" )
                bsSendContactName.value =Addresses[0] ; //联系人    }
...
selectGCByKey=function() {
	...
	if (window.event.keyCode==13) {
			...
			var Addresses = lastSelCell.innerText.split("|");
            if( Addresses.length < 3 )
            {
            }
            else
...
lightGCByKey=function() {
	...
	var tmpObj=document.getElementById("hb");
    ...
    try
    {
	    ...
	    for(var i=0;i<inputObjs.length;i++){
		    if (inputObjs[i].type == "text" && inputObjs[i].getAttribute("isGCCombox")=="yes"){
			    ...
			    new bsGCCombox({gccomboxID: inputObjs[i].id,gcpostUrl: bindPostUrl,gcrelationObj: bindRelationObj,gcevent:bindEvent});
		    }
	    }
    }
    catch(err) 
